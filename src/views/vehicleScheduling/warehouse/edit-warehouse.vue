...
<script lang="ts">
    ...
    @Component
    export default class EditWarehouse extends AbpBase{
        ...
        save(){
            (this.$refs.Form as any).validate(async (valid:boolean)=>{
                if(valid){
                  this.$Modal.confirm({
                      title:'是否确认修改？',
                      content:'区域：['+this.repository.location.areaName+']<br/>仓库类型：['+this.GetRepositoryTypeName(this.repository.repositoryType)+']<br/>仓库名称：['+this.repository.repositoryName+']',
                      okText:'确认',
                      cancelText:'取消',
                      onOk:async()=>{
