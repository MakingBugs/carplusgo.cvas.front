<template>
    <div>
        ...
        >
            <Form ref="batchAddForm"  label-position="top" :rules="batchAddRule" :model="batchAddRequest">
                <FormItem label="保险预设配置" prop="insurancePresetId">
                    <Select v-model="batchAddRequest.insurancePresetId" clearable @on-change="getExportList">
...
<script lang="ts">
    ...
    @Component({})
    export default class BatchAdd extends AbpBase{
        ...
        async getExportList(){
            ...
            var insurancePreset = (this.insurancePresetList as Array<any>).find(x=>x.id===this.batchAddRequest.insurancePresetId);
            if(!!insurancePreset){
        ...
        getListFromInsurancePreset(insurancePreset){
            this.data.map(x=>{
                (insurancePreset.presetInsuranceType as Array<Number>).map(y=>{
                    var insuranceDetail = (this.insuranceDetailList as Array<any>).find(z=>z.insuranceTypeId===y && z.insurancePolicyId == x.id);
                    var insuranceType:InsuranceType = (!!insuranceDetail) ? insuranceDetail.insuranceType : (this.insuranceTypeList as Array<any>).find(z=>z.id===y);
                    this.list.push({
                        carBaseId:x.carBaseId,
                        carNo:x.carBase.carNo,
                        insuranceTypeId:insuranceType.id,
                        insuranceTypeName:insuranceType.name,
                        insuranceOperationType:'批加',
                        insuranceContractType:this.getInsuranceContractTypeName(x.insuranceContractType),
                        insurancePolicyType:this.getInsurancePolicyTypeName(x.insurancePolicyType),
                        supplierId:x.supplierId,
                        supplierName:x.supplier.fname,
                        insuranceNum:x.insuranceNum,
                        serialNumber:'',
                        insuredAmount:(!!insuranceDetail) ? insuranceDetail.insuredAmount : '',
                        originalAmount:(!!insuranceDetail) ? insuranceDetail.originalAmount : '',
                        transactionAmount:(!!insuranceDetail) ? insuranceDetail.transactionAmount : '',
                        noDeductibleOriginalAmount:(!!insuranceDetail) ? insuranceDetail.noDeductibleOriginalAmount : '',
                        noDeductibleTransactionAmount:(!!insuranceDetail) ? insuranceDetail.noDeductibleTransactionAmount : '',
                        remark:'',
                    })
                })
            })
        ...
        async save(){
            ...
            (this.$refs.batchAddForm as any).resetFields();
