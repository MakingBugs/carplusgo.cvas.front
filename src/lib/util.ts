...
class Util{
    ...
    setPermissionsByRole(permissions:Array<any>,role:any){
        return permissions.map(value=>{
            if(!!value.children&&value.children.length>0){
                value.children = this.setPermissionsByRole(value.children,role);
                value.checked=false;
            }else if(role.permissions.includes(value.name)){
                value.checked=true;
            }else{
                value.checked=false;
            }
    ...
    getPermissionsByTree(treeData:Array<any>,permissions:Array<string>){
        treeData.map(value=>{
            if(!!value.children&&value.children.length>0){
                this.getPermissionsByTree(value.children,permissions);
                if(value.indeterminate||value.checked){
...
     * @return yyyy-MM-dd
     */
    formatExcelDate(day:number) {
        var a=typeof day;//获取数据类型
        if(a=='number'){
